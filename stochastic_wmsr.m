...
%according to W_MSR algorithm    
    ...
    v1 = [normrnd(0,1) normrnd(1,2) normrnd(2,3)];
    v2 = [normrnd(0,1) normrnd(1,2) normrnd(2,3)];
%     v1 = v1/norm(v1);
%     v2 = v2/norm(v2);
    
    vector_similarity = fun_cos_similarity(v1,v2);
    % cosine similarity returns value close to 1 if vectors are similar
    if(vector_similarity >= spoof_threshold)
        spoof_flag = 1;
    else
        spoof_flag = 0;
    end
